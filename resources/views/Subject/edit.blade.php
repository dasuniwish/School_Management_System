...
<div class="d-flex justify-content-center align-items-center mt-5">
    <div class="p-4 bg-light rounded shadow" style="width: 400px;">
        <form action="{{ route('subjects.update', $subjects->id) }}" method="POST" class="w-100 px-4 py-4" enctype="multipart/form-data">
            @csrf
            @method('PUT')

            <div class="mb-3">
                <label class="form-label text-dark fw-bold">Subject Code</label>
                <input name="subject_code" class="form-control" type="text" value="{{ old('subject_code', $subjects->subject_code) }}">
                @error('subject_code')
            ...
            <div class="mb-3">
                ...
                <input name="subject_name" class="form-control" type="text" value="{{ old('subject_name', $subjects->subject_name) }}">
                @error('subject_name')
                    <p class="text-danger text-sm">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="mb-3">
                ...
                <textarea name="description" class="form-control" rows="2">{{ old('description', $subjects->description) }}</textarea>
                @error('description')
                    <p class="text-danger text-sm">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="mb-3">
                ...
                <select name="teacher_id" class="form-control">
                    ...
                    @foreach($teachers as $teacher_id => $name)
                        <option value="{{ $teacher_id }}" {{ old('teacher_id', $subjects->teacher_id) == $teacher_id ? 'selected' : '' }}>
                            {{ $name }}
                        </option>
                    @endforeach
                </select>
